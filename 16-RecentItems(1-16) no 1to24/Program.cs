...
namespace _16_RecentItems
{
    public static class Program
    {
        public class Globals
        {
            ...
            public static Vector2u tiless = new Vector2u(3, 3);
            ...
            public static bool omode = false;
        ...
        public class table1 : Drawable
        {
            public bool loadd()
            {
                t1.PrimitiveType = SFML.Graphics.PrimitiveType.Lines;
                t1.Resize(Globals.sx * Globals.sy * 2);
                for (uint j = 0; j <= Globals.sx * 4; ++j)
                {
                    uint index1 = j * 2;
                    t1[index1] = new Vertex(new Vector2f(j * 3, 0), new SFML.Graphics.Color(150, 150, 150));
                    t1[index1 + 1] = new Vertex(new Vector2f(j * 3, Globals.sy * 12), new SFML.Graphics.Color(150, 150, 150));
                }
                return true;
            }
        ...
        public class table1y : Drawable
        {
            public bool loadd()
            {
                t1.PrimitiveType = SFML.Graphics.PrimitiveType.Lines;
                t1.Resize(Globals.sx * Globals.sy * 2);
                for (uint j = 0; j <= Globals.sy * 4; ++j)
                {
                    uint index1 = j * 2;
                    t1[index1] = new Vertex(new Vector2f(0, j * 3), new SFML.Graphics.Color(150, 150, 150));
                    t1[index1 + 1] = new Vertex(new Vector2f(Globals.sx * 12, j * 3), new SFML.Graphics.Color(150, 150, 150));
                }
                return true;
            }
        ...
        static void Main()
        {
            ...
            table1 xs = new table1();
            table1y xss = new table1y();
            ...
            while (Globals.frm.Visible)
            {
                ...
                if (Globals.create == true)
                {
                    ...
                    if (Globals.omode == true)
                    {
                        Globals.renderwindow.Draw(Globals.map1);
                    }
                    if (Globals.showg == true)
                    {
                        Globals.renderwindow.Draw(xs);
                        Globals.renderwindow.Draw(xss);
                    }
        ...
        static void paint(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                if (Globals.omode == false)
                {
                    ...
                    uint posp = (iyy - 1) * (Globals.sx * 4) + ixx;

                    if (Globals.nowpaint == true)
                    {
                        if (0 <= posp && posp <= 16 * Globals.sx * Globals.sy && worldPos.X <= Globals.sx * 12 && worldPos.Y <= Globals.sy * 12 && worldPos.X >= 0 && worldPos.Y >= 0)
                        {
                            if (Globals.level[(int)posp - 1] != (uint)Globals.curcolor)
                            {
                                Globals.level[(int)posp - 1] = (uint)Globals.curcolor;
                    ...
                    else if (Globals.nowdimmer == true)
                    {
                        if (0 <= posp && posp <= 16 * Globals.sx * Globals.sy && worldPos.X <= Globals.sx * 12 && worldPos.Y <= Globals.sy * 12 && worldPos.X >= 0 && worldPos.Y >= 0)
                        {
                            if (Globals.level[posp - 1] > 0)
                            {
                                uint dimm = Globals.level[posp - 1] - 1;
                                Globals.level[(int)posp - 1] = dimm;
                                Globals.savechange = true;
                            }
                        }
                    }
                    ...
                    Globals.map.load("tileset.png", Globals.tiless, Globals.level, Globals.sx * 4, Globals.sy * 4);
                }
                else if (Globals.omode == true)
                {
                    Vector2i pixelpos = new Vector2i(e.X, e.Y);
                    Vector2f worldPos = Globals.renderwindow.MapPixelToCoords(pixelpos);
                    ...
                    {
                        if (Globals.levelshut[posp - 1] != Globals.shutnow)
                        {
                            Globals.levelshut[(int)posp - 1] = (uint)Globals.shutnow;
                        }
                        else if (Globals.levelshut[posp - 1] == Globals.shutnow)
                        {
                            Globals.levelshut[(int)posp - 1] = 0;
                        }
                        Globals.savechange = true;
                    }
                    Globals.map1.load("tileset.png", Globals.opensize, Globals.levelshut, Globals.sx, Globals.sy);
                }
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (Globals.nowpaint == true)
                {
                    if (0 <= posp && posp <= 16 * Globals.sx * Globals.sy && worldPos.X <= Globals.sx * 12 && worldPos.Y <= Globals.sy * 12 && worldPos.X >= 0 && worldPos.Y >= 0)
                    {
                        Globals.level[(int)posp - 1] = (uint)Globals.curcolor1;
                        Globals.savechange = true;
                    }
                }
                if (Globals.nownorpaint == true)
        ...
        static void tell(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            if (Globals.create == true)
            {
                ...
                int posp1 = ((int)(worldPos1.Y / 3)) * ((int)Globals.sx * 4) + (int)(worldPos1.X / 3);
                ...
                if (posp1 >= 0 && posp1 <= Globals.sx * Globals.sy * 16)
                {
